...
if [ $# -eq 0 ]; then
	# Launch the shell
	docker exec -it $DB_DOCKER /bin/bash -c "mysql -u$DB_USER -p$DB_PWD -D $DB_DATABASE"
# Else launch a shell
else
	# Launch the argument
	docker exec -it $DB_DOCKER /bin/bash -c "mysql -u$DB_USER -p$DB_PWD -D $DB_DATABASE -e \"$@\""
fi
