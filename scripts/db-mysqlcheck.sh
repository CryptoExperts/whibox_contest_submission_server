...
# Launch the shell
docker exec -it $DB_DOCKER /bin/bash -c "mysqlcheck $DB_DATABASE user -u$DB_USER -p$DB_PWD"
docker exec -it $DB_DOCKER /bin/bash -c "mysqlcheck $DB_DATABASE program -u$DB_USER -p$DB_PWD"
