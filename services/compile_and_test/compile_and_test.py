...
def main():
    ...
    try:
        max_ram = 5000 + 2**10 * int(os.environ['CHALLENGE_MAX_MEM_COMPILATION_IN_MB']) # in kiB
        max_cpu_time = 10 + int(os.environ['CHALLENGE_MAX_TIME_COMPILATION_IN_SECS']) # in seconds
        ...
        subprocess.run('%s; %s; %s'%(cmd_ulimit_ram, cmd_ulimit_cpu_time, cmd_gcc), check=True, shell=True)
    except:
    ...
    try:
        subprocess.run(['gcc', '/main.o', path_to_o, '-o', path_to_executable], check=True)
    except:
    ...
    ciphertexts = b''
    max_ram = 5000 + 2**10 * int(os.environ['CHALLENGE_MAX_MEM_EXECUTION_IN_MB']) # in kiB
    max_cpu_time = 10 + int(os.environ['CHALLENGE_NUMBER_OF_TEST_VECTORS']) * int(os.environ['CHALLENGE_MAX_TIME_EXECUTION_IN_SECS']) # in seconds
    cmd_ulimit_ram = 'ulimit -v %d'%(max_ram)
    cmd_ulimit_cpu_time = 'ulimit -t %d'%(max_cpu_time)
